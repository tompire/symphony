defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server auto-accepts MCP tool call elicitations when approval policy is never" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-mcp-tool-elicitation-auto-approve-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-mcp-tool-elicitation-auto-approve.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-mcp-tool-elicitation-auto-approve.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' "$line" >> "$trace_file"
        ...
        case "$count" in
          ...
          4)
            ...
            printf '%s\\n' '{"id":120,"method":"mcpServer/elicitation/request","params":{"_meta":{"codex_approval_kind":"mcp_tool_call","persist":["session","always"],"tool_title":"execute_sql"},"message":"Allow Supabase to run tool \\"supabase_execute_sql\\"?","mode":"form","requestedSchema":{"type":"object","properties":{}},"serverName":"codex_apps","threadId":"thread-719","turnId":"turn-719"}}'
            ;;
          5)
            printf '%s\\n' '{"method":"turn/completed"}'
            exit 0
      ...
      issue = %Issue{
        id: "issue-mcp-tool-elicitation-auto-approve",
        identifier: "MT-719",
        title: "Auto approve MCP tool call elicitation",
        description: "Ensure MCP tool call approval elicitations continue automatically",
        state: "In Progress",
